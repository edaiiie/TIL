  ...
  return (
    <div className="flex flex-col">
      ...
      <section className="bg-white py-20 lg:py-24">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold text-[#1e3a5f]">Our Team</h2>
            <Link
              href="/people"
              className="inline-flex items-center gap-1 text-sm font-medium text-[#1e3a5f] hover:underline"
            >
          ...
          <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-5">
            {/* Professor Card - Larger */}
            <Link
              href="/professor"
              className="group col-span-1 rounded-xl border border-slate-200 bg-[#f8f9fa] p-6 transition-all hover:border-[#1e3a5f]/30 hover:shadow-md sm:col-span-2 lg:col-span-2"
            >
              <div className="flex items-start gap-5">
                <div className="relative h-20 w-20 shrink-0 overflow-hidden rounded-xl bg-[#1e3a5f]/10">
                  <Image
                    src={professor.image ?? "/images/lab-photo.jpg"}
                    alt={professor.name}
                    fill
                    className="object-cover"
                  />
                ...
                <div>
                  ...
                  <h3 className="mt-1 text-lg font-semibold text-[#1e3a5f] group-hover:text-[#2a4a73]">
                    {professor.name}
                  </h3>
                  {professor.email && (
            ...
            {teamMembers.map((member, index) => (
              <Link
                key={member.id}
                href="/people"
                className="group rounded-xl border border-slate-200 bg-white p-5 transition-all hover:border-[#1e3a5f]/30 hover:shadow-md"
              >
                <div className="relative mx-auto flex h-16 w-16 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-[#1e3a5f]/20 to-[#1e3a5f]/5">
                  {member.image ? (
                    <Image
                      src={member.image}
                      alt={member.name}
                    ...
                    <span className="text-xl font-semibold text-[#1e3a5f]/60">
                      {member.name.split(" ").pop()?.charAt(0)}
                    </span>
                  )}
                </div>
                <div className="mt-4 text-center">
                  <h3 className="font-semibold text-[#1e3a5f] group-hover:text-[#2a4a73]">
                    {member.name}
                  </h3>
                  <p className="mt-0.5 text-xs text-[#888]">{member.role}</p>
                </div>
              </Link>
            ))}
          </div>
